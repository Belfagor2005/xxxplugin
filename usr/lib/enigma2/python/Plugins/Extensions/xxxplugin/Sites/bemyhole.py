...
currversion = '1.0'
title_plug = 'Bemyhole '
desc_plugin = ('..:: Bemyhole by Lululla %s ::.. ' % currversion)
...
print(current)
print(parent)
pluglogo = os.path.join(PLUGIN_PATH, 'pic/bemyhole.png')
stripurl = 'aHR0cHM6Ly93d3cuYmVteWhvbGUuY29tLw'
_session = None
Path_Movies = '/tmp/'
global search
search = False
...
class main(Screen):
    ...
    def filterChannels(self, result):
        if result:
            ...
            try:
                search = True
                self.session.open(bemyhole2, name, url)
            except:
                return
        else:
            self.resetSearch()
    ...
    def keyNumberGlobalCB(self, idx):
        ...
        if sel == 'SEARCH':
            namex = sel.upper()
            lnk = 'https://www.bemyhole.com/search/'
            self.search_text(namex, lnk)
        if sel == ("Bemyhole Tags"):
            namex = "bemyhole"
            lnk = 'https://www.bemyhole.com/tags/'
            self.session.open(bemyhole, namex, lnk)
        # elif sel == ("Bemyhole Last"):
            # namex = "last"
            # lnk = 'https://www.bemyhole.com/latest-shemale-porn/'
            # self.session.open(bemyhole, namex, lnk)
        # elif sel == ("bemyhole Channels"):
            # namex = "channels"
            # lnk = 'https://www.bemyhole.com/channels/videos/'
            # self.session.open(bemyhole2, namex, lnk)
        # elif sel == ("bemyhole More Videos"):
            # namex = "videos"
            # lnk = 'https://www.bemyhole.com/porn/'
            # self.session.open(bemyhole2, namex, lnk)
        else:
            return
...
class bemyhole(Screen):
    ...
    def play_that_shit(self, url, name):
        self.session.open(bemyhole2, str(name), str(url))

    def exit(self):
        self.close()


class bemyhole2(Screen):
    def __init__(self, session, name, url):
        self.session = session
        Screen.__init__(self, session)
        skin = os.path.join(skin_path, 'defaultListScreen.xml')
...
class bemyhole4(Screen):
    ...
    def play_that_shit(self, url, name):
        self.session.open(bemyhole5, str(name), str(url))
...
class bemyhole5(Screen):
    ...
    def cat(self):
        ...
        try:
            ...
            if six.PY3:
                content = six.ensure_str(content)
            regexcat = "embed_mode.*?video_url: '(.*?)'"
            match = re.compile(regexcat, re.DOTALL).findall(content)
            url = match[0]
            url = url.replace("&amp;", "&").replace("/?embed=true", "")
            # print('url b5= ', url)
            name = self.name
            # for url, name in match:
                # name = self.name
