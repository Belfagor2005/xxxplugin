...
print(current)
print(parent)
pluglogo = os.path.join(PLUGIN_PATH, 'pic/easyporn.png')
stripurl = 'aHR0cHM6Ly93d3cuaGQtZWFzeXBvcm4uY29t'
_session = None
Path_Movies = '/tmp/'
global search
search = False
...
class main(Screen):
    ...
    def filterChannels(self, result):
        if result:
            ...
            try:
                search = True
                self.session.open(easyporn2, name, url)
            except:
                return
        else:
            self.resetSearch()
    ...
    def keyNumberGlobalCB(self, idx):
        ...
        if sel == ("Easyporn top rated"):
            ...
            namex = "popular"
            lnk = 'https://www.hd-easyporn.com/most-viewed-porn-movies/'
            self.session.open(easyporn2, namex, lnk)
        elif sel == ("Easyporn categories"):
            namex = "categories"
...
class easyporn(Screen):
    ...
    def cat(self):
        if "categories" in self.name.lower():
            ...
            self.session.open(easyporn2, self.name, self.url)
        return
...
class easyporn2(Screen):
    ...
    def ok(self):
        ...
        name = self.names[idx]
        url = self.urls[idx]
        self.session.open(easyporn3, name, url)
...
class easyporn3(Screen):
    ...
    def play_that_shit(self, url, name):
        self.session.open(easyporn4, str(name), str(url))

    def exit(self):
        self.close()


class easyporn4(Screen):
    def __init__(self, session, name, url):
        self.session = session
        Screen.__init__(self, session)
        skin = os.path.join(skin_path, 'defaultListScreen.xml')
